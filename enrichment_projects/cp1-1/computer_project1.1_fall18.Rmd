...
#need to use bootstrap method to come up with prediction interval
#for both the hazard rate, and survival function at an arbitrary point
#function takes a data.frame and returns a bootstrapped sample
create_bs_dataframe <- function(df){
  df_nobs <- nrow(df)
  bs_df <- df[sample(x = 1:nrow(df),size = nrow(df),replace = T),]
  return(bs_df)
}
#### Run bootstrap replications
B <- 1000

#ignore frail measure - only consider sex for bootstrap stuff
# gender_ph <- coxph(Surv(time, status) ~ sex, data =  kidney)
# new_dat = data.frame(time = 2, sex = 2) #female
# survfit(gender_ph, newdata = new_dat)
# baseline <- basehaz(gender_ph)
# # Draw baseline hazard (that's male)
# plot(baseline$time, baseline$hazard, type='l',main="Hazard rates") 
# # Draw female hazard
# lines(baseline$time, exp(gender_ph$coefficients)*baseline$hazard, col="blue") 

bs_res <- do.call('rbind',
                  lapply(1:B, function(a){
                    bs_df <- create_bs_dataframe(df = kidney) 
                    bs_ph <- coxph(Surv(time, status) ~ sex, data =  bs_df)
                    return(data.frame(beta= bs_ph$coefficients, rep_n = a))
                  }))

#percentile method of the beta estimate (gender impact against baseline)
#not symmetric

#ggplot(bs_res, aes(x = bs_res$beta)) + geom_density() + theme_minimal()

#quantile method - but not appropriate for non symmetric bootstrap results
beta_upper <- quantile(bs_res$beta,probs = 0.95)
beta_lower <- quantile(bs_res$beta, probs = 0.05)
#also need to consider interval for bootstrap prediction (at fixed point in time??)

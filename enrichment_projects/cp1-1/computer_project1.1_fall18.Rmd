...
head(stanford2)
```
For this exercise, I located a dataset in the survival package, that describes the Stanford Heart Transplant data in a different format. It has five variables:

1) id:	ID number
2)time:	survival or censoring time
3)status:	censoring status
4)age:	in years
5)t5:	T5 mismatch score 

First, I compute the univariate Cox analyses for the two variables; then we’ll fit multivariate cox analyses using two variables to describe how the factors jointly impact on survival.
```{r PH-regression_un, echo = T, include = T, fig.align='center',fig.height=4}
res.cox <- coxph(Surv(time, status) ~ age, data = stanford2)
res.cox
res.cox.zph <- cox.zph(res.cox)
res.cox.zph
res.cox <- coxph(Surv(time, status) ~ t5, data = stanford2)
res.cox
res.cox.zph <- cox.zph(res.cox)
res.cox.zph
```
...
1) Statistical significance, z, gives the Wald statistic value,evaluates whether the beta coefficient of a given variable is statistically significantly different from 0. From the output above, we can conclude that the variable age has highly statistically significant coefficients, but t5 doesn't. So, following we just focus on age.
...
So, it gives us the predict about the hazard ratio for any given variables. for example, the variable age whose p_value is less than 0.0061, with a hazard ratio HR = 1.03.
...
library(survminer)
res.cox <- coxph(Surv(time, status) ~ age, data =  stanford2)
ggsurvplot(survfit(res.cox), data = stanford2, palette = "#2E9FDF",ggtheme = theme_minimal())
new_dat = data.frame(age = c(35))

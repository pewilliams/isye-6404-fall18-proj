...
1) *Kernel Regression,*
```{r Kernel Regression, echo = T, include = T, fig.align='center',fig.height=4}
...
```
2) *Local Polynomial Regression,* 
```{r Local Polynomial Regression, echo = T, include = T, fig.align='center',fig.height=4}
...
```
3) *LOESS,*    
``` {r Package Installation, echo = T, include = T, fig.align='center',fig.height=4}
...
#Install "stats" package to be able to perform loess and spline data smoothing.
install.packages('stats', dependencies=TRUE, repos='http://cran.rstudio.com/')
```
#Loess models, with varying spans, with degree=1. When interpreting the summary results
#for the loess models, we will notice that increasing span will decrease the SSR
#but the curve of the model will become more jagged, making it so that the curve may not
#be the best representation. Loess is mostly a visual tool to see curves for non-linear curves through
#the use of data to form the line rather than a specified mathematical model. 
#Loess is based purely on the span and degree of the polynomial for the potential loess model.
```{r Loess Models, echo = T, include = T, fig.align='center',fig.height=4}
...
```{r Plot of Mortality, echo = T, include = T, fig.align='center',fig.height=4}
...
lines(loessfit4$x,loessfit5$y,col="red", lwd=2)
```
...
2) Local polynomial regression overcomes the above problem by fitting a higher degree polynomial here. What's more, Local polynomial regression also has other advantage such as,independence of the regression design (ﬁxed or random, highly clustered or nearly uniform), and eﬃciency in an asymptotic minimax sense.

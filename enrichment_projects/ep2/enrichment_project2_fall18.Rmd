...
the two variables mortality and education, but our sample size is not particularly large, and oftentimes loess fitting requires quite a large sample size to utilize effectively. 
...
For LOESS models, cross-validation is a procedure commonly used to determine an optimally performing bandwidth with which our sum of squared errors would be reduced while retaining a smooth fit along the data (preferable to jagged fits). The way to 
...
There will be a different Confidence Interval Estimate for different values of bandwidth and amplitude.

•	Find Bootstrap Samples
For the given dataset find 10000 bootstrap samples and calculate the probability distribution using Kernel Density estimator.

•	Pointwise Confidence Intervals
Using the bootstrap estimates of probability density at each point calculate the 95% Confidence Interval at each point.

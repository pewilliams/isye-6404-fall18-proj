...
``` {r set_var_names, echo = T, include = T, fig.align='center',fig.height=4}
...
#Recall our plot with the mortality and education data.
...
j=order(Education)
lines(Education[j],loess.model.1$fitted[j],col="blue", lwd=2)
lines(Education[j],loess.model.2$fitted[j],col="green", lwd=2)
lines(Education[j],loess.model.3$fitted[j],col="yellow", lwd=2)
lines(Education[j],loess.model.4$fitted[j],col="red", lwd=2)
```
...
```{r Loess_Comparison, echo = T, include = T, fig.align='center',fig.height=4}
#The loess.smooth() function allows us to make fitted lines for the smoothed lines with varying spans
...
loess.auto.fit=loess(Mortality~Education,span=0.919,degree=1

plot(Mortality~Education)
lines(Education[j],loess.model.1$fitted[j],col="blue", lwd=2)
lines(Education[j],loess.auto.fit2$fitted[j],col="red",lwd=2)
```
...
plot(Mortality~Education)
spline.mortality=smooth.spline(Education,Mortality, cv= F)
j=order(spline.mortality$x)
lines(spline.mortality$x[j],spline.mortality$y[j], col="purple", lwd=2)
```
...
loess_cv_as <- unlist(mclapply(1:nrow(rdat), function(i){
    train_data <- rdat[-i,]
    train_x <- matrix(train_data$Education)
    test_rec <- rdat$Mortality[i]
    model <- loess.as(y = train_data$Mortality, x = train_x, degree=1, criterion = 'gcv')
    cv_eval <- matrix(Education[i])
    squared_error <- (Mortality[i] - predict(model, newdata = cv_eval ) )^2
    return(squared_error)
},mc.cores = 10))
...
#Smoothing Spline Plot using the smooth.spline() function in {stats} package
#plot(Mortality~Education)
spline.mortality <- smooth.spline(Education,Mortality, cv= F) #why cv=TRUE??
...
ggplot() + geom_point(data = rdat, aes(x = Education, y = Mortality)) + 
  ...
  geom_line(data = sp_dat, aes(x = spx, y = spy , color = 'navy')) + theme(legend.position = 'none') + 
  ggtitle('Mortality vs. Education, with Smoothing Spline')
```
...
i) *Exercise the FIVE versions of CIs stated in CP-1 for the survival function $S(x)$ give in 1) above. Compare these CIs. Moreover, focus on the MEDIAN ($50^{th}$ percentile), and evaluate the histogram of the bootstrap-estimates, bootstrap-bias and -standard-error. Comment about the computational results.*
...
The bias is calculated by taking the difference of the mean of bootstrap estimates and the original estimate from the Kaplan Meier estimator for median survival time. It turns out that the calculated value of bias is $-4.3$ Days.
...
The Standard error from the bootstrap estimates is $55$ days. This is in agreement with our normal Confidence Estimates.
...
ggplot(aes(x=time,y=value, color = variable ,group = variable), data=melted_km_dat1) +
 geom_step(size = 1.25) + theme_minimal() + xlab('Time (days)') +
 ylab('Survival Function') +

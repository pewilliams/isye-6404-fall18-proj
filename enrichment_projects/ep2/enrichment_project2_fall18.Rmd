...
```{r km_estimate, echo = T, include = T, fig.align='center',fig.height=4}
bw <- npregbw(formula=hdat$Status~hdat$Days, tol=.1, ftol=.1)
...
points(hdat$Days, hdat$Status, cex=.2, col="red")

plot(NA, xlim = c(min(hdat$Days), max(hdat$Days)), ylim = c(0, 1))
lines(locpoly(hdat$Days, hdat$Status, bandwidth=2, degree=1), col=3)
```

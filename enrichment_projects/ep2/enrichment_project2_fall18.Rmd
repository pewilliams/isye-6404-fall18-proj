...
Ruixuan Zhang & Part of Cross-validation Procedure \\ 
\hline
\end{tabular}
\end{center}
...
##CV calculation of Kernal Reg
	...
	for(i in (1:n)){
	 y = Mortality[-i]
	 x = Education[-i]
	 modelwithoutone <- npreg(txdat=x,tydat=y,gradients = TRUE)
...
```{r lp_cv, include = T, echo = F}
	...
	for(i in (1:n)){
	 y2 = Mortality[-i]
	 x2 = Education[-i]
	 modelwithoutone2 <- locfit(y2~x2)
	 cvs2[i] = (Mortality[i] - predict(modelwithoutone2, newdata=data.frame(x=x2[i])))^2
	}
	##calculation of the mean of cvs -- the CV value as defined.
...
x3=Education
n=length(Mortality)
cvs3=vector(mode="numeric",length(Mortality))
##Looping procedure of taking one data point out for prediction, and then the calculation of the CV criterion through the formula
##(1/n) sum(yi-yi_prediction)^2 through each possible iteration
for(i in (1:n)){
 y3 = Mortality[-i]
 x3 = Education[-i]
 modelwithoutone3 <- loess(y3~x3, span=0.25, degree=2)
 cvs3[i] = (Mortality[i] - predict(modelwithoutone3, newdata=data.frame(x3=x3[i])))^2
}
...
mean(cvs3,na.rm=TRUE)

##Resetting the variables for calculating CV criterion for the automated span model
y4=Mortality
x4=Education
cvs4=vector(mode="numeric",length(Mortality))
##
for(i in (1:n)){
 y4 = Mortality[-i]
 x4 = Education[-i]
 modelwithoutone4 <- loess.as(Education,Mortality,degree=2,"gcv")
 cvs4[i] <- (Mortality[i] - predict(modelwithoutone4, newdata=data.frame(x4=x4[i])))^2
}
##Calculation of CV criterion for our 2nd model using LOO-CV
mean(cvs4,na.rm=TRUE)
```

...
loess_cv <- unlist(mclapply(1:nrow(rdat), function(i){
    train_data <- rdat[-i,]
    test_rec <- rdat$Mortality[i]
    model <- loess(Mortality~Education, span=0.25, degree=2, data=  train_data)
    cv_eval <- data.frame(Education = Education[i])
    squared_error <- (Mortality[i] - predict(model, newdata = cv_eval ) )^2
    return(squared_error)
},mc.cores = 10)) 
...
loess_cv_as <- unlist(mclapply(1:nrow(rdat), function(i){
    ...
    test_rec <- rdat$Mortality[i]
    model <- loess.as(y = train_data$Mortality, x = train_x, degree=2, criterion = 'gcv')
...
cv_res_df <- data.frame(
  Model = c('Kernel Regression','Local Polynomial','Loess Span = (.25), Degree = 2','Loess Auto Span, Degree = 2'),
  SquaredError = round(c(mean(kernel_reg_cv),mean(local_poly_cv),
                            mean(loess_cv,na.rm = T),mean(loess_cv_as, na.rm=T)),digits = 0),
  stringsAsFactors = F
)

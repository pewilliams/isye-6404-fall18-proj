...
```
We then compute these statistics using $B=1000$ bootstrap samples based on our original dataset, which consists of `r nrow(rider_data)` observations. Code and details are below. As shown in the table of results, bootstrap sample standard deviations for spearman's rank were more dispersed than for other statistics.  
...
We can see with these histograms that the data are definitely not distributed normally, so at least one of the assumptions thus far is met. Despite this, the Signed Rank Test (for two samples) looks at the median difference between paired values, and our data are not paired in this case. 
...
#Checking shapes of distribution (differences between ridership for a given day)
par(mfrow=c(1,2))
plot(density(rider_data$bus))
plot(density(rider_data$rail_boardings))
```
...
The results of the Wilcoxon Ranked Sum Test show the p-value of the test to be small enough to be significant, meaning we reject the null hypothesis that the distributions of the two samples are the same. The fact that the Wilcoxon Ranked Sum Test and the Mann Whitney U Test are essentially the same test allows us to also state that the medians of the two distributions are also different. 
...
As seen in the histograms and Q-Q plots for the ridership data, both of our two samples (bus ridership and rail boardings) do not follow a normal distribution, but they both have fairly large sample sizes. In utilizing the two sample t-test, some assumptions must first be met. One such assumption is an underlying normal distribution, which we can see is not the case with our current data. Despite this, we will continue with our performance of the Welch's t-test. Welch's t-test is a robust test similar to the student's t-test, with the advantage that it accounts for unequal variances. Welch's t-test is also more appropriate for more skewed data as compared to the standard two sample t-test. Similar to the nonparametric tests performed (Mann-Whitney U), the data must be continuous.   

```{r welch test, include = T, echo = F}
#Performing Welch's T-test with assumed unequal variance
t.test(rider_data$bus,rider_data$rail_boardings, var.equal=FALSE)
```
  
As we can see from the results of the test, the p-value is quite low, showing that there is a significant difference between the two means. In comparing the two tests, the parametric t-test tests for significance in difference between the means of the two samples, while the nonparametric tests (Mann-Whitney U) test for significance in differences between the distributions (and medians) of the two samples. In terms of results, both the Mann-Whitney U test and Welch's t-test with assumed unequal variance show a significant difference in mean and median for the two samples. Despite the fact that they both could determine whether there are differences between the two sample distributions, the Mann-Whitney U test is a more reliable test if only for the fact that the Mann-Whitney U test is better suited for nonnormal distributions (e.g. the ridership counts). As opposed to Welch's t-test, which still holds the assumption of normality (similar to the student's t-test)

...
```
If we plot the density functions, we can see the distributions seem to resemble normal distributions, though they do resemble each other in terms of their "shape". 
...
The test statisic U and W end up becoming the same value, used in the same approximately normal distribution as the W_{n}, which means we can use the same tail probability to find the p-value.
***
